...
if ! hash java 2>/dev/null; then
    echo 'ERROR! You do not have the Java Runtime Environment installed, please install Java JRE from https://adoptium.net/?variant=openjdk21&package=jre and try again.' >&2
    exit 1
fi
...
if [ "$JAVA_VERSION" -lt 21 ]; then
    echo 'ERROR! HiveMQ requires at least Java version 21.' >&2
    exit 1
fi
...
# Reduce excessive native memory usage due to fragmentation caused by concurrent de-/allocations when using glibc
